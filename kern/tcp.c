...
tcp_send(struct tcp_virtual_channel* channel, struct tcp_pkt* pkt, size_t length) {
    ...
    pkt->hdr.src_port = htons(channel->host_side.port);
    pkt->hdr.dst_port = htons(channel->guest_side.port);
    pkt->hdr.win_size = htons(sizeof(channel->buffer));

    struct ip_pkt result = {};
    struct ip_hdr* hdr = &result.hdr;

    hdr->ip_protocol = IP_PROTO_TCP;
    hdr->ip_source_address = htonl(channel->host_side.ip);
    hdr->ip_destination_address = htonl(channel->guest_side.ip);
    ...
    uint32_t network_data_length = htons(data_length);
    memcpy((void *)buf + 8 + 2, (void *)&network_data_length, sizeof(network_data_length));
    pkt->hdr.checksum = htons(ntohs(ip_checksum(buf, data_length + 12)) - channel->host_side.port);
...
tcp_recv(struct ip_pkt* pkt) {
    if (ntohs(pkt->hdr.ip_total_length) - IP_HEADER_LEN < TCP_HEADER_LEN) {
